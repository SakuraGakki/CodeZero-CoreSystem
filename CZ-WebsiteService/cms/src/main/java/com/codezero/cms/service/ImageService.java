...
public class ImageService {
    ...
    public BaseResponse<ImageResponse> bookImageUpload(MultipartFile file){
        BaseResponse baseResponse = new BaseResponse();
        if(file.isEmpty()){
            baseResponse.setStatus(-1);
            baseResponse.setMessage("上传图片文件为空！！");
        ...
        String imageExtension = imageName.substring(imageName.lastIndexOf("."));
        //应用场景
        String sceneType = "book";
        String sceneName = "书籍配图";
        if(imageExtension.contains("md")||imageExtension.contains("pdf")){
            sceneName = "书籍内容";
        }
        //图片大小
        Long imageSize = file.getSize();
        //图片路径
        String imageUrl = imageUploadUrl+"images/book/"+imageName;
        //图片类型
        String type = "0";
        //上传时间
        String date = DateUtils.getCurrentTime();
    ...
    public BaseResponse<ImageResponse> bookImageReplace(MultipartFile file,String imgId){
        BaseResponse baseResponse = new BaseResponse();
        if(file.isEmpty()){
            baseResponse.setStatus(-1);
            baseResponse.setMessage("上传图片文件为空！！");
